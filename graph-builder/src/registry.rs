...
pub fn fetch_releases(
    ...
    username: Option<&str>,
    password: Option<&str>,
    cache: &mut HashMap<String, Option<Release>>,
...
///
/// Tags and release metadata are immutable, thus each
/// tagged release is looked up at most once and both
/// positive and negative results cached indefinitely.
fn cache_release(
    ...
    if let Some(release) = cache.get(&tag) {
    ...
    cache.insert(tag, release.clone());
    Ok(release)
}
